				...
				if (mthis.hasOwnProperty(fieldName)) {
					angular.forEach(this.$validations[fieldName], function(validationValue, functionName) {
						if (functionName != "required" && mthis[functionName]) {
							...
							switch(typeof mthis[fieldName]) {
								case "string":
								case "number":
									props = [mthis[fieldName]];
									break;
								default:
									props = mthis[fieldName];
									res = true;
							...
							angular.forEach(props, function(prop) {
								if (keepGoing) {
									res = mthis[functionName](prop, value);
									if (res !== true) {
										if (!errorMessage) errorMessage = mthis.$validationErrorMessages[functionName] || "is invalid";
										if (angular.isFunction(errorMessage)) errorMessage = errorMessage(fieldName, prop, value);
